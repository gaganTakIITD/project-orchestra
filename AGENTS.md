...
**Demo mode + Clerk.** The app runs without Clerk by default (demo mode, no `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`); the API client just sends an `X-Orchestra-Role` header and the backend (`AUTH_MODE=demo`) resolves a seeded user. Clerk hooks/components are only mounted when a publishable key is set — `lib/use-orchestra-auth.ts` guards `clerkEnabled` *before* calling Clerk's `useAuth()` (calling it in demo mode would throw "useAuth can only be used within `<ClerkProvider>`" with `@clerk/nextjs` 7.5.17). Keep that guard first if you edit that file.
